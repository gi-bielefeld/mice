...
def graph_1_degree_contraction(graph, repeats=set()):
    ...
    while i < len(Q):
        ...
        if len(graph_new[u]) > 1:
            #this should exist only because of the removing of the selfloops
            continue 
        if len(graph_new[u]) == 0:
            #print("already merged", v, u)
            continue
        ...
        #if v in graph_new[u2] or get_other_ext(v) in graph_new[u2]:
        #    #no going back to the same node
        #    continue
        ...
        #print(v,"--",u,"==",u2,"--", graph_new[u2], sep="")
        for w in graph_new[u2]:
            #if u2 in graph_new[w]:
        ...
        #print(f"merged {u} {v}")
        #print(f"v {v}:", graph_new[v])
        #print(f"u {u}", graph_new[u])
        #print(f"u2 {u2}", graph_new[u2])
